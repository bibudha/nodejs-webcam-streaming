...
function concatenateInLinuxOrMac(files) {
    ...
    ffmpegCommand.on('end', function() {
        ...
        blobSvc.createBlockBlobFromLocalFile('videos', 'videoblobstorage', final_file, function(error, result, response){
          if(!error){
            console.log("isError");
            console.log(error);
          }
    ...
    /*blobSvc.createContainerIfNotExists('videos', function(error, result, response){
        if(!error){
